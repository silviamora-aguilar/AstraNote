...
| 2026-06-03 | REV-2026-06-03C | Right-sized BL-21 for localhost single-user MVP | Deferred SMR-02, SMR-08, and SMR-12 to Post-MVP; simplified SMR-04, SMR-07, and SMR-11 while keeping core serviceability guardrails in MVP |
...
- **SRG-21 ✅**: After 15 minutes of inactivity, private-note unlock state shall expire and re-authentication shall be required.
- **SRG-22 ✅**: Failed unlock attempts shall return clear error messages, shall not crash the app, and shall apply rate limiting after 5 consecutive failures.
- **SRG-23 ✅**: After 5 consecutive failed unlock attempts within a single session, the private-note unlock mechanism shall enter a locked-out state for a minimum of 5 minutes. Each subsequent lockout period shall double (exponential backoff). For the current MVP baseline, lockout state is session-scoped and resets across app restarts.
- **SRG-24 ✅**: Unlock error responses shall be identical in content and timing regardless of whether the failure is caused by a wrong PIN or an internal error, preventing enumeration of failure cause.
- **SRG-25 ✅**: The only fields permitted to remain unencrypted in local storage are: `note_id`, `created_at`, `updated_at`, `is_private` (boolean), `is_deleted` (boolean), and `deleted_at`. All other note fields, including `title`, `body`, and `version_content`, shall be encrypted at rest per SRG-01. Any future addition of a plaintext field requires explicit update to this allowlist in requirements before implementation.
- **SRG-26 ✅**: The encryption key used for private notes shall be derived from the user PIN using PBKDF2-HMAC-SHA256 with a minimum iteration count of 260,000, a randomly generated 16-byte salt stored alongside the derived key material, and a 256-bit output key. The raw PIN shall never be stored or logged. The derived key shall be held only in memory for the duration of the unlocked session.
- **SRG-27 ✅**: The private-note unlock factor shall be an app-wide 4-digit numeric PIN (`0000`-`9999`) with a bootstrap default of `1234` until explicitly changed by the user from the UI settings flow.
- **SRG-28 ✅**: The app shall provide an in-app PIN change workflow that requires entering the current PIN, validates the new 4-digit PIN and confirmation, and re-encrypts existing private-note payloads to the new PIN atomically. The settings workflow shall stage input as verify-current-PIN first, then reveal new/confirm PIN fields only after verification, and shall render a completion state after successful update. The unlock UI shall support keypad-based 4-digit entry with masked circle indicators and automatic submit after the fourth digit.
