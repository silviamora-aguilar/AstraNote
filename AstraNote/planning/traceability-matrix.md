...
| REQ-24 | Limit-reached blocking behavior + message | `NoteService`, stable machine-readable error mapping | UC: Create Note | API/HTMX error mapping layer | ⚠️ Partially Traced | Implemented error path; pending UX copy standardization artifact |
...
| NFR-04 | Optimistic concurrency / version field | `Note` (version), stable `STALE_VERSION` error code mapping | Activity: conflict/save error branch | — | ✅ Fully Traced | |
