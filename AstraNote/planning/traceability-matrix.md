...
| REQ-02 | Title/body validation rules | `Note` (title, body fields), `NoteService` validators | UC: Create Note | — | ⚠️ Partially Traced | Implemented; validation branches are not yet diagrammed in UML |
...
| REQ-20 | Bold/italic/underline formatting | `Note` (body) | UC: Edit Note | — | ⚠️ Partially Traced | Implemented UI detail; formatting controls are not yet represented in UML |
| REQ-21 | Formatting safety (no title modification, no data loss) | `Note` | UC: Edit Note | — | ⚠️ Partially Traced | Implemented constraints; safety rules are not yet explicit in UML |
| REQ-22 | Formatting storage rules (Markdown markers) | `Note` (body storage) | — | Shared DB persistence artifact | ⚠️ Partially Traced | Implemented storage behavior; Markdown marker rules need explicit UX/spec artifact |
