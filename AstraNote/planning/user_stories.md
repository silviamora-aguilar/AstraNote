...
Organization rule: requirement-mapped stories are ordered exactly as in planning/requirements.md, with all MVP stories first and all Post-MVP stories second.

## MVP Requirement-Mapped User Stories
...
- If a persistence write fails, the note remains intact and user receives an error message.
...
Scope convention: NFR acceptance criteria inherit the scope tag in each NFR heading ([MVP] or [Post-MVP]).
...
**User Story**: As a user, I want the app to handle thousands of notes in the active dataset so I can rely on it long term.
...
### SRG-04.1 — Localhost Transport Exception [MVP]
**Requirement**: Localhost-only development on 127.0.0.1 may use HTTP during MVP development.

**User Story**: As a local MVP user, I want the app to run on localhost without requiring deployment-grade TLS setup.

**Acceptance Criteria**:
- Localhost development on 127.0.0.1 is supported over HTTP for MVP workflows.
- Local MVP usage does not require non-local transport configuration.
...
### SRG-08 — Immutable Version Records [Post-MVP]
...
**Requirement**: Restore operations shall preserve original note ID and create an audit entry for restore action. Version-history continuity remains deferred until SRG-08 is activated.
...
**Requirement**: No non-local feature that transmits note content may be released unless SRG-04.2 transport encryption requirements are satisfied.
...
- Code review and DoD checklists include a SRG-04.2 confirmation step for all non-local network-touching features.
...
**Requirement**: The application shall write diagnostic (non-audit) log entries to a rotating log file with UTC timestamp, severity level, and message. Tier and correlation metadata are recommended for MVP and required by Post-MVP hardening. Log file rotates at 5 MB retaining two prior files.
...
- Each entry includes UTC timestamp, severity (DEBUG/INFO/WARNING/ERROR), and message; tier and correlation metadata are included when available in the MVP baseline.
...
**Requirement**: On startup, if the primary persistence store is unreadable or structurally invalid, the app shall log the error and fail fast with a clear startup message rather than running in a partially functional state.

**User Story**: As a user, I want startup to fail clearly when storage is unreadable so I am not left in a partially functional state.
...
- If the persistence store is present but structurally invalid, startup fails with a clear message and no partial startup.
- The integrity failure is logged at ERROR level.
...
**Requirement**: The app shall embed a semantic version string accessible at runtime and include it in startup/runtime metadata outputs. A dedicated About/Help UI surface is deferred to Post-MVP.
...
## Post-MVP Requirement-Mapped User Stories

### SRG-04.2 — Non-Local Data In-Transit Protection [Post-MVP]
**Requirement**: Any non-local network transmission containing note content (including sync, backup, restore, and export/import APIs) shall use TLS 1.2 or higher; note content shall not be transmitted over unencrypted channels outside localhost.

**User Story**: As a user, I want non-local note content protected in transit so network interception cannot expose my data.

**Acceptance Criteria**:
- Non-local network paths carrying note content enforce TLS 1.2+.
- Requests over insecure non-local transport are rejected.
- Sync/backup/restore/export-import routes are covered by transport security tests before release.
...
### NFR-01 — Concurrency Capacity [Post-MVP]
...
### NFR-02 — Active Session Definition and Workload [Post-MVP]
...
### NFR-03 — Concurrency Latency Targets [Post-MVP]
...
### NFR-04 — Concurrent Edit Conflict Policy [Post-MVP]
...
### NFR-05 — Overload Behavior [Post-MVP]
...
### NFR-10 — Browser Keyboard-Only Workflows [Post-MVP]
...
### NFR-12 — Platform Input Reachability and Visibility [Post-MVP]
...
### SRG-29 — Retention Expiry Purge [Post-MVP]
**Requirement**: At retention expiry (15 days after soft delete), notes in Trash shall be automatically and permanently purged.
...
### SMR-02 — Configurable Log Level [Post-MVP]
...
### SMR-08 — Storage Schema Version Guard [Post-MVP]
...
### SMR-12 — Graceful Shutdown [Post-MVP]
...
### WEB-09 — Persistent User Account Model [Post-MVP]
...
### WEB-10 — Server-Side Session Management [Post-MVP]
...
### WEB-11 — Separate Login Password and Note Passphrase [Post-MVP]
...
## Backlog-Only Stories (Not Requirement-Mapped)
